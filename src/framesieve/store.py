...
from .index import FrameIndex, IndexStats, _segment
...
def build_store(video: str, encoder: SiglipEncoder, out: str, *,
                ...
                gpu_decode: bool = False, seed: int = 0,
    ...
    with ThreadPoolExecutor(jpeg_workers) as pool:
        for ts, frames in stream:
            t = time.perf_counter()
            # the encoder wants square model-resolution input; the store wants
            # the frame as shot, so resize only for the encoder
            small = np.stack([np.array(_resize(f, size)) for f in frames])
            emb = encoder.encode_frames(torch.from_numpy(small)).cpu().numpy()
            t_encode += time.perf_counter() - t

            t = time.perf_counter()
            jp = list(pool.map(lambda f: _encode_jpeg(f, jpeg_quality,
                                                      store_max_pixels), frames))
            t_jpeg += time.perf_counter() - t

            batches.append(pa.record_batch(
                [pa.array(np.arange(n, n + len(frames)), pa.int32()),
                 pa.array(ts, pa.float64()),
                 pa.array(np.zeros(len(frames), np.int32), pa.int32()),
                 pa.array(list(emb.astype(np.float32)),
                          pa.list_(pa.float32(), encoder.spec.dim)),
                 pa.array(jp, pa.large_binary())], schema=schema))
            ts_all.append(ts.astype(np.float32))
            emb_all.append(emb.astype(np.float32))
            n += len(frames)

    emb_cat = (np.concatenate(emb_all) if emb_all
               else np.zeros((0, encoder.spec.dim), np.float32))
    seg = _segment(emb_cat, segment_tau)

    # write the segmentation back into the batches now that it is known
    fixed = []
    off = 0
    for b in batches:
        m = b.num_rows
        cols = list(b.columns)
        cols[2] = pa.array(seg[off:off + m], pa.int32())
        fixed.append(pa.record_batch(cols, schema=schema))
        off += m
    ...
    if os.path.exists(out):
        import shutil
        shutil.rmtree(out)
    lance.write_dataset(pa.Table.from_batches(fixed, schema=schema), out,
    ...
    stats = IndexStats(
        video=os.path.abspath(video), duration_s=info.duration_s,
        target_fps=target_fps, n_frames=n, n_encoded=n,
        n_segments=int(seg.max() + 1) if len(seg) else 0,
        ...
        realtime_factor=info.duration_s / wall if wall else 0.0, seed=seed)
    meta = {"stats": asdict(stats), "store_bytes": size_bytes,
...
class FrameStore:
    ...
    def __init__(self, path: str, decode_workers: int = 8):
        ...
        self.stats = IndexStats(**self.meta["stats"])
    ...
    def emb(self) -> np.ndarray:
        ...
        if self._emb is None:
            ...
            self._emb = np.stack(col.to_numpy(zero_copy_only=False)).astype(np.float16)
        return self._emb
